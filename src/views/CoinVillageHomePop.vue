...
export default {
    ...
    computed: {
        ...
        computInvestment() {
            ...
            if(this.computComonCoinObj && this.computComonCoinObj['rows']){
                ...
                //获取抛硬币游戏的好友返利
                investment = coinArr && coinArr[0] && coinArr[0].investment || 0;
            }
            return investment;
        },
        //计算我的投资列表历史数据
        computMydevestmentList(){
            ...
            if(this.computComonCoinObj && this.computComonCoinObj['rows']){
                ...
                //获取抛硬币游戏的好友返利
                myDevestmentList = coinArr && coinArr[0] && coinArr[0].investhistorys || [];
            }
        ...
        computRewardpoolAmount(){
            ...
            if(this.computGbdataCoinObj && this.computGbdataCoinObj['rows']){
                let tempArr = that.computGbdataCoinObj['rows'] || [];
                let rewardpool = tempArr && tempArr[0] && tempArr[0].rewardpool || 0;
                let feepool = tempArr && tempArr[0] && tempArr[0].feepool || 0;
                //计算奖池总数  
    ...
    methods: {
        ...
        doDividend() {
            ...
            //撤资操作
            that.computScatterClient.contract(that.coinConfig.gameContract, { requiredFields })
            .then(hello => {
                console.log(hello);
        ...
        promiseAlldone() {
            ...
            Promise.all([playerPro]).then(value => {
                ...
                // 处理玩家投资表返回的数据
                const accountObj1 = tempList1 && tempList1.rows && tempList1.rows.filter(item => item.owner == that.username);
                const tempPercent = (accountObj1 && accountObj1[0] && parseFloat(accountObj1[0].invest_rate)) || 0;
                // console.log('----------------tempPencent---------',tempPercent);
                if (tempPercent == 0) {
        ...
        getPlayerDivest() {
            ...
            const playerDivestPromise = new Promise((resolve, reject) => {
                that.eosClient.getTableRows({
                    ...
                    scope:  that.coinConfig.toContract,
                    table: 'invest',
                    limit: 100,
                    lower_bound: 0,
                }).then(result => {
